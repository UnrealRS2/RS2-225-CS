...
public class Jagfile
{
    ...
    public byte[]? read(string name, byte[]? dst)
    {
        ...
        for (var i = 0; i < upper.Length; i++) hash = hash * 61 + upper.ToCharArray()[i] - 32;

        for (var i = 0; i < fileCount; i++)
            if (fileHash[i] == hash)
            {
                if (dst == null) dst = new byte[fileUnpackedSize[i]];

                if (unpacked)
                {
                    if (fileUnpackedSize[i] >= 0) Array.Copy(buffer, fileOffset[i], dst, 0, fileUnpackedSize[i]);
                }
                else
                {
