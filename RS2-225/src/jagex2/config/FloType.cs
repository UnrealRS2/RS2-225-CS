...
public class FloType
{
    ...
    public static void unpack(Jagfile config)
    {
        ...
        if (instances == null) instances = new FloType?[count];

        for (var id = 0; id < count; id++)
        {
            if (instances[id] == null) instances[id] = new FloType();
    ...
    private void setColor(int rgb)
    {
        ...
        if (min != max)
        {
            if (l < 0.5D) s = (max - min) / (max + min);
            if (l >= 0.5D) s = (max - min) / (2.0D - max - min);

            if (red == max)
                h = (green - blue) / (max - min);
            else if (green == max)
                h = (blue - red) / (max - min) + 2.0D;
            else if (blue == max) h = (red - green) / (max - min) + 4.0D;
        }
        ...
        var hue = this.hue + (int)(Math.random() * 16.0D) - 8;
        switch (hue)
        {
            case < 0:
                hue = 0;
        ...
        var saturation = this.saturation + (int)(Math.random() * 48.0D) - 24;
        switch (saturation)
        {
            case < 0:
                saturation = 0;
        ...
        var lightness = this.lightness + (int)(Math.random() * 48.0D) - 24;
        switch (lightness)
        {
            case < 0:
                lightness = 0;
